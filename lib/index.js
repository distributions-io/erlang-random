...
*	Creates a matrix or array filled with Erlang random variables.
*
...
function random( dims, options ) {
	...
	if ( ndims === 1 ) {
		...
		if ( dt === 'generic' ) {
			return array( len, k, lambda, rand );
		}
		return typedarray( len, dt, k, lambda, rand );
	}
	// Multidimensional data structures...
	if ( dt !== 'generic' ) {
		if ( ndims === 2 ) {
			return matrix( dims, dt, k, lambda, rand );
		}
		// TODO: dstructs-ndarray support goes here. Until then, fall through to plain arrays...
	}
	return arrayarray( dims, k, lambda, rand );
} // end FUNCTION random()
